...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Save { name } => {
            ...
            config.save()?;
            println!(
                "Saved profile '{}' for current hardware configuration.",
                name
            );
        }
...
fn apply_profile(
    ...
    let mut interface_map: std::collections::HashMap<String, String> =
        ...
        if let Some(interface) = interface_map.get(&mon_conf.stable_id) {
            let cmds = vec![
                vec![
                    "position".to_string(),
                    "set".to_string(),
                    mon_conf.x.to_string(),
                    mon_conf.y.to_string(),
                ],
                // Mode setting in Niri: mode <width>x<height>
                vec![
                    "mode".to_string(),
                    format!("{}x{}", mon_conf.width, mon_conf.height),
                ],
            ...
            for mut args in cmds {
                // Prepend common args
                let mut full_args =
                    vec!["msg".to_string(), "output".to_string(), interface.clone()];
...
fn apply_hyprland(
    ...
    for mon_conf in &profile.monitors {
        if let Some(interface) = interface_map.get(&mon_conf.stable_id) {
            ...
            eprintln!(
                "Warning: Monitor {} in profile but not connected",
                mon_conf.stable_id
            );
        }
    }
