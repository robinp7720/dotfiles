...
## Highlights
- **Hyprland setup** with shared configuration, multi-monitor layouts (`hypr/monitor_layouts`), and scripts for switching between default, game, and external display modes.
- **BSPWM workflow** including focus automation, per-mode screenlayout scripts, and integration with `sxhkd`, `polybar`, and `nitrogen`.
- **Dynamic theming via Matugen** that renders matched color palettes for Hyprland, Waybar, Kitty, Rofi, and Dunst from a single template directory.
- **Productivity bars**: Polybar modules cover GPU-friendly clipboard-to-GPT prompts, pacman/AUR update counters with notifications, bluetooth, MPD, and more; Waybar styles live alongside Wayland configs.
...
3. **Run the linker**
   ...
   ./setup.sh
   ```
   The script will link `zshrc` to `~/.zshrc` and replace the directories listed in `setup.sh` inside `~/.config/`.
...
- `waybar/` – Waybar configuration and CSS theme generated via Matugen.
- `matugen/` – Template files and `config.toml` describing how colors propagate across Hyprland, Waybar, Kitty, and Dunst.
- `rofi/`, `kitty/`, `dunst/`, `termite/` – Application-specific themes.
- `scripts/` – Utility scripts such as `update.sh` (runs `paru`/`yay` with notifications) and `update-notification.sh` (launches an update terminal via Dunst actions).
...
- **Theme generation**: After installing Matugen, run it to regenerate color schemes across Waybar, Hyprland, Kitty, and Dunst using the templates in `matugen/templates/`.
