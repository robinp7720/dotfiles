...
if [[ "${REFIND_MATUGEN_ESCALATED:-0}" != "1" && ! -w "${theme_dir}" ]]; then
  if ! command -v sudo >/dev/null 2>&1; then
    echo "sudo is required to update ${theme_dir}" >&2
    exit 1
  fi
...
"${img_cmd[@]}" -size 1920x1080 "gradient:${bg_lowest}-${bg_dim}" "${theme_dir}/banner.png"
# Selection: flat fill; rEFInd applies its own overlay on hover.
"${img_cmd[@]}" -size 256x96 "xc:${primary}" "${theme_dir}/selection_dark-big.png"
"${img_cmd[@]}" -size 144x48 "xc:${primary}" "${theme_dir}/selection_dark-small.png"
